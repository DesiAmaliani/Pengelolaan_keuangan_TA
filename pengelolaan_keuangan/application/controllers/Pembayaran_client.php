...
class Pembayaran_client extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Pembayaran_client_model');
    ...
    public function index()
    {
        ...
        $config['total_rows'] = $this->Pembayaran_client_model->total_rows($q);
        $pembayaran = $this->Pembayaran_client_model->get_limit_data($config['per_page'], $start, $q);
        ...
        $data = array(
            ...
            "container" => "client/pembayaran/pembayaran_list",
    ...
    {
        $row = $this->Pembayaran_client_model->get_by_id($id);

        if ($row) {
            $data = array(
                'button' => 'Bayar',
                'action' => site_url('pembayaran_client/update_action'),
                ...
                "header" => "client/header","nav" => "client/nav",
                "container" => "client/pembayaran/pembayaran_form",
                'user'=>$this->db->GET_WHERE('client',['username' => $this->session->userdata('username')])->row_array()
        );
        $this->load->view("template", $data);
        } else {
            ...
            redirect(site_url('client/pembayaran'));
        }
    }
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Pembayaran_client_model->update($this->input->post('id_pem', TRUE), $data);
            $this->session->set_flashdata('message','<div class="alert alert-success"><center><b>
            Pembayaran Berhasil</b></center></div>');
            redirect(site_url('pembayaran_client/index'));
        }
    }
    public function invoice($id)
	{
		...
		$data = array(
                ...
                'bulan' => set_value('bulan', $row->bulan),
                'status' => set_value('status', $row->status),
					"container" => "client/pembayaran/invoice",
					'user'=>$this->db->GET_WHERE('client',['username' => $this->session->userdata('username')])->row_array());
    ...
    public function invoice_print($id)
	{
		...
		$data = array(
                ...
                'bulan' => set_value('bulan', $row->bulan),
                'status' => set_value('status', $row->status),
					"container" => "client/pembayaran/invoice_print",
					'user'=>$this->db->GET_WHERE('client',['username' => $this->session->userdata('username')])->row_array());
    ...
    $this->form_validation->set_rules('bukti_pem', 'bukti_pem', 'trim');
