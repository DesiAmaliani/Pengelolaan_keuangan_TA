                  ...
                  <div class="card-body p-0">
                    <div class="table-responsive">
                      <table class="table table-striped">
                        ...
                        <?php
                        $pembayaran_data= $this->db->query('SELECT * FROM pembayaran inner join paket on pembayaran.id_paket=paket.id_paket inner join client on pembayaran.id_client=client.id_client Where pembayaran.id_client ="'.$user['id_client'].'"');
                        ...
                        foreach($total->result() as $total){ echo $pembayaran->bandwith.' ( '.$total->nama_jp.' )'; } ?></td>
                            ...
                            echo "Belum Bayar"; 
                            echo "<td>".anchor(site_url('pembayaran_client/bayar/'.$pembayaran->id_pem),'Bayar')."</td>"; 
                            }elseif($pembayaran->status==1){
                                echo "Menunggu Konfirmasi";
                            }elseif($pembayaran->status==2){
                                echo "Sudah dikonfirmasi";
                                echo "<td>".anchor(site_url('pembayaran_client/invoice/'.$pembayaran->id_pem),'Invoice')."</td>"; 
                            }else{
