                  ...
                  <div class="card-body p-0">
                    <div class="table-responsive">
                      <table class="table table-striped">
                        ...
                        <?php
                        $pembayaran_data= $this->db->query("SELECT * FROM pembayaran inner join paket on pembayaran.id_paket=paket.id_paket inner join client on pembayaran.id_client=client.id_client Order by 'DESC'");
                        ...
                        foreach($total->result() as $total){ echo $pembayaran->bandwith.' ( '.$total->nama_jp.' )'; } ?></td>
                            ...
                            <td>
                            <a class="perbesar" href="<?php echo base_url(); ?>tampilan/pembayaran/<?php echo $pembayaran->bukti_pem ?>"> <img width="100" src="<?php echo base_url(); ?>tampilan/pembayaran/<?php echo $pembayaran->bukti_pem ?>" alt="(Belum ada foto)" width="80" class="shadow-light"></a></td>
                            <td><div class="badge badge-primary"><?php 
                            if($pembayaran->status==0){
                            echo "Belum Bayar"; 
                            }elseif($pembayaran->status==1){
                                echo "Menunggu Konfirmasi"."<td>";
                                echo anchor(site_url('transaksi/konfirmasi/'.$pembayaran->id_pem),'Konfirmasi','onclick="javasciprt: return confirm(\'Are You Sure ?\')"')." | "; 
                                echo anchor(site_url('transaksi/tolak/'.$pembayaran->id_pem),'Tolak','onclick="javasciprt: return confirm(\'Are You Sure ?\')"')."</td>"; 
                            }elseif($pembayaran->status==2){
                                echo "Sudah dikonfirmasi";
                                echo "<td>".anchor(site_url('transaksi/invoice/'.$pembayaran->id_pem),'Invoice')."</td>";
                            }else{
